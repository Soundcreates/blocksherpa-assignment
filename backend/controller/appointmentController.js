    ...
    if (recipientEmail) {
      try {
        const mailOptions = {
          ...
          to: recipientEmail,
          subject: `Viewing Appointment ${status.charAt(0).toUpperCase() + status.slice(1)} - REChain`,
    ...
    if (recipientEmail) {
      try {
        const mailOptions = {
          from: process.env.EMAIL,
          to: recipientEmail,
          subject: 'Viewing Scheduled - REChain',
    ...
    if (recipientEmail) {
      try {
        const mailOptions = {
          from: process.env.EMAIL,
          to: recipientEmail,
          subject: 'Appointment Cancelled - REChain',
    ...
    if (recipientEmail) {
      try {
        const mailOptions = {
          from: process.env.EMAIL,
          to: recipientEmail,
          subject: 'Meeting Link Updated - REChain',

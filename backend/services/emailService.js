...
class EmailService {
  constructor() {
    this.fromAddress = process.env.EMAIL_USER || 'noreply@REChain.com';
  }
  ...
  async sendWelcomeEmail(userEmail, userName) {
    const subject = 'Welcome to REChain - Your Account is Ready!';
  ...
  async sendEmailVerification(userEmail, userName, verificationUrl) {
    const subject = 'Verify Your REChain Email Address';
  ...
  async sendPasswordResetEmail(userEmail, resetUrl) {
    const subject = 'Reset Your REChain Password';
  ...
  async sendNewsletterWelcome(userEmail) {
    const subject = 'Welcome to REChain Newsletter 📧';
  ...
  async sendAppointmentNotificationToAdmin(appointment, userDetails) {
    ...
    const adminEmail = process.env.ADMIN_EMAIL || 'admin@REChain.com';
  ...
  async sendUserSuspended(userEmail, userName, days, reason, suspendedUntil) {
    const subject = `REChain Account Suspended - ${days} Day${days > 1 ? 's' : ''}`;
  ...
  async sendUserBanned(userEmail, userName, reason) {
    const subject = 'REChain Account Permanently Banned';
  ...
  async sendUserReactivated(userEmail, userName) {
    const subject = 'REChain Account Reactivated - Welcome Back!';
